...
func marketsConsumer(conn *amqp.Connection) {
	...
	go func() {
		for d := range msgs {
			...
			fmt.Println("Consumed market from Queue for Fixture: ", marketSet.FixtureId)
					...
					for _, vals := range unmarshalselections {
						...
						for market_id, group := range groupedByMarketId {
							for _, oddsObj := range group {
								...
								fmt.Println("Markets Updated in DB for Fixture: ", marketSet.FixtureId, ": ", markets.Name, ": ", oddsObj.Odds)
